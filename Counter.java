...
public class Counter {
	...
	public static Map<String,Integer> countBigrams(String filename) {
		Map<String,Integer> countBigramsMap = new HashMap<String,Integer>();
		try (BufferedReader in = new BufferedReader(new FileReader(filename))) {
			String line;
			while ((line = in.readLine()) != null) {
				String[] words = line.split(" ");
				for (String word : words) {
					...
					for (int i = 0; i < word.length() - 1; i++) {
						...
						if (c0 >= 'a' && c0 <= 'z' && c1 >= 'a' && c1 <= 'z') {
							...
							if (countBigramsMap.containsKey(bigram)){
								countBigramsMap.put(bigram, countBigramsMap.get(bigram) + 1);
							} else {
								countBigramsMap.put(bigram, 1);
							}
						}
					}
				}				
			}
			return countBigramsMap;
